...
- Browser-engine bugs that affect every extension equally.

## Privacy & Transparency

Link Scrubber is a privacy tool. The build output is intentionally
**unminified** with **source maps** included so users can audit the deployed
JavaScript without having to trust the build chain. The repository's CI
workflow checks that no external URLs are present in the source; if you
spot one in a build that wasn't there in source, that's a security report.
