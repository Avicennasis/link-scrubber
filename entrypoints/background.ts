...
export default defineBackground(() => {
  ...
  browser.storage.onChanged.addListener(async (changes, areaName) => {
    // Only our config matters, and it lives in storage.sync: ignore other
    // areas and non-config keys so we don't re-read and re-broadcast to every
    // tab on unrelated storage activity (FR-266/FR-274).
    if (areaName !== 'sync' || !changes.config) return;
